...
namespace Menelia.Client.AdminMenu
{
    ...
    {
        private static List<CustomVehicle> _customVehicles = new List<CustomVehicle>();
        ...
        private static void spawnMenu(UIMenu supMenu)
        {
            ...
            _customVehicles.Add(new CustomVehicle("polraiden", typeof(Vehicles.Emergency)));
            _customVehicles.Add(new CustomVehicle("menswat", typeof(Vehicles.Emergency)));
            _customVehicles.Add(new CustomVehicle("policegt350r", typeof(Vehicles.Emergency)));
        ...
        private static void spawn(UIMenu supMenu/*, string optionName*/, Type classType, float distance)
        {
            var menu = MainMenu.MenuPool.AddSubMenu(supMenu, /*optionName*/ classType.Name.Replace("_", " "));
            
            menu.MouseEdgeEnabled = false;
            menu.ControlDisablingEnabled = false;

            var list = new List<object>();
            foreach (var obj in Enum.GetValues(classType))
            {
                list.Add(obj);
            }
            ...
            {
                var vehicleName = Enum.GetName(classType, obj);
                ...
                menu.OnItemSelect += async (sender, item, index) =>
                {
                    if (item == vehicleItem)
                    {
                        ...
                        var enumObj = Convert.ToUInt32(obj);
            ...
            foreach(var cv in _customVehicles)
            {
                if (cv.category.Equals(classType))
                {
                    var vehicleName = cv.name;
                    var vehicleItem = new UIMenuItem(vehicleName, $"Spawn le véhicule: {vehicleName}");
                    menu.AddItem(vehicleItem);
                    menu.OnItemSelect += async (sender, item, index) =>
                    {
        ...
        private static void modificationMenu(UIMenu supMenu)
        {
            ...
            menu.OnMenuStateChanged += (oldMenu, newMenu, state) =>
            {
                if(state == MenuState.Opened || state == MenuState.ChangeForward)
                {
                    ...
                    var list = new List<VehicleModType>();
                    ...
                    foreach (var type in sortedList)
                    {
                        ...
                        for (var i = 0; i < veh.Mods[type].ModCount; i++)
                        {
                            options.Add(i);
                        }

                        var modName = Enum.GetName(typeof(VehicleModType), type);
